...
function TradingPage({ ticker, currentPrice }) {
        ...
        const fetchPortfolioData = async () => {
            for (const portfolioID of ids) {
                try {
                    ...
                    const cashResponse = await fetch(`https://tradesim-api.adityakmehrotra.com/paper_trader/portfolio/get/cash?id=${portfolioID}`);
                    if (!cashResponse.ok) {
    ...
    const fetchBuyingPower = (portfolioID) => {
        fetch(`https://tradesim-api.adityakmehrotra.com/paper_trader/portfolio/get/cash?id=${portfolioID}`)
            .then(res => res.json())
    ...
    const fetchSharesOwned = (portfolioID, code) => {
        fetch(`https://tradesim-api.adityakmehrotra.com/paper_trader/portfolio/get/assetsMap/shares?id=${portfolioID}&code=${code}`)
            .then(res => res.json())
            .then(shares => {
    ...
    const handleConfirmOrder = () => {
        fetch("https://tradesim-api.adityakmehrotra.com/paper_trader/transaction/get/nextTransactionID")
            ...
            .then(transactionID => {
                ...
                .then(response => response.json())
                .then(() => {
                    fetch(`https://tradesim-api.adityakmehrotra.com/paper_trader/portfolio/add/transaction?id=${selectedPortfolioID}&transactionID=${transactionID}`, {
                        method: "POST"
                    })
                    .then(() => {
                        setInputValue('');
