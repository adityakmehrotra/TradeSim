...
function TradingPage({ ticker, currentPrice }) {
    ...
    const fetchPortfolios = (ids) => {
        ...
        const fetchPortfolioData = async () => {
            for (const portfolioID of ids) {
                try {
                    const nameResponse = await fetch(`https://parcel-sides-effort-italiano.trycloudflare.com/paper_trader/portfolio/get/name?id=${portfolioID}`);
                    if (!nameResponse.ok) {
                        throw new Error(`Error fetching name for portfolio ID ${portfolioID}`);
                    }
                    const name = await nameResponse.text();

                    const cashResponse = await fetch(`https://parcel-sides-effort-italiano.trycloudflare.com/paper_trader/portfolio/get/cash?id=${portfolioID}`);
                    if (!cashResponse.ok) {
    ...
    const fetchBuyingPower = (portfolioID) => {
        fetch(`https://parcel-sides-effort-italiano.trycloudflare.com/paper_trader/portfolio/get/cash?id=${portfolioID}`)
            .then(res => res.json())
    ...
    const fetchSharesOwned = (portfolioID, code) => {
        fetch(`https://parcel-sides-effort-italiano.trycloudflare.com/paper_trader/portfolio/get/assetsMap/shares?id=${portfolioID}&code=${code}`)
            .then(res => res.json())
            .then(shares => {
    ...
    const handleConfirmOrder = () => {
        fetch("https://parcel-sides-effort-italiano.trycloudflare.com/paper_trader/transaction/get/nextTransactionID")
            .then(response => response.json())
            .then(transactionID => {
                const orderDetails = {
                    transactionID,
                ...
                fetch("https://parcel-sides-effort-italiano.trycloudflare.com/paper_trader/transaction/create", {
                    method: "POST",
                    headers: {
                ...
                .then(response => response.json())
                .then(() => {
                    fetch(`https://parcel-sides-effort-italiano.trycloudflare.com/paper_trader/portfolio/add/transaction?id=${selectedPortfolioID}&transactionID=${transactionID}`, {
                        method: "POST"
                    })
                    .then(() => {
                        setInputValue('');
