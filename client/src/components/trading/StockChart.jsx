        ...
        <div className="chart-title">
          ...
          <div className="chart-price-info">
            ...
            <span className={`price-diff ${priceChange >= 0 ? 'positive' : 'negative'}`}>
              {priceChange >= 0 ? '+' : ''}
              {priceChange.toFixed(2)}({((priceChange / chartData[0].price) * 100).toFixed(2)}%)
            </span>
            {hoverInfo && <span className="hover-date">{formatTime(hoverInfo.timestamp)}</span>}
          </div>
        </div>
        <div className="time-range-selector">
          {['1D', '1W', '1M', '3M', 'YTD', '1Y', '5Y'].map((range) => (
      ...
      >
        ...
        <div className="chart-svg-container" ref={svgContainerRef}>
          ...
          >
            ...
            <path d={areaPath} fill={chartAreaColor} className="chart-area" />
            ...
            {[0, 0.25, 0.5, 0.75, 1].map((ratio, i) => {
              const index = Math.min(
                Math.floor((chartData.length - 1) * ratio),
                chartData.length - 1
              );
              const x = index * xStep;
          ...
          {hoverInfo && (
            <div className="hover-line" style={{ left: `${hoverInfo.xPosition}%` }}>
              <div className="hover-price-indicator">${hoverInfo.price.toFixed(2)}</div>
            </div>
          )}
        </div>
        ...
        <div className="chart-x-labels">
          {chartData.length > 1 &&
            [0, 0.25, 0.5, 0.75, 1].map((ratio, i) => {
              const index = Math.min(
                Math.round((chartData.length - 1) * ratio),
                chartData.length - 1
              );
              return (
