...
export default function AppLayout() {
    ...
    const handleSearch = async (ticker) => {
        try {
            await axios.get(`https://parcel-sides-effort-italiano.trycloudflare.com/paper_trader/security/suggestion/${ticker}`);
    ...
    const updateSuggestions = async (value) => {
        if (value.length > 1) {
            try {
                const response = await axios.get(`https://parcel-sides-effort-italiano.trycloudflare.com/paper_trader/security/suggestion/${value}`);
