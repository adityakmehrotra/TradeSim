...
export default function PortfolioDetails() {
  ...
  const fetchAllPortfolios = () => {
    fetch(`http://tradesim-api.adityakmehrotra.com/paper_trader/account/get/portfolioList?id=${accountID}`)
      .then(response => response.json())
      .then(data => {
        if (Array.isArray(data)) {
          const portfolioPromises = data.map(id =>
            fetch(`http://tradesim-api.adityakmehrotra.com/paper_trader/portfolio/get/name?id=${id}`)
              .then(response => {
                if (response.ok) {
  ...
  const fetchPortfolioDetails = (id) => {
    fetch(`http://tradesim-api.adityakmehrotra.com/paper_trader/portfolio/get?id=${id}`)
      .then(response => response.json())
      .then(data => {
        setPortfolio(data);
        if (data.assets) {
  ...
  const fetchAssetPrices = (assets, cashAmount) => {
    const assetPromises = Object.keys(assets).map(asset => {
      if (asset === "Cash") {
        ...
        return fetch(`http://tradesim-api.adityakmehrotra.com/paper_trader/polygon/price?ticker=${asset}`)
          .then(response => {
            if (!response.ok) {
  ...
  const fetchTransactions = (transactionIDs) => {
    const transactionPromises = transactionIDs.map(id =>
      fetch(`http://tradesim-api.adityakmehrotra.com/paper_trader/transaction/get?id=${id}`)
        .then(response => response.json())
    );
  ...
  const handleConfirmDelete = () => {
    ...
    .then(response => {
      if (response.ok) {
        return fetch(`http://tradesim-api.adityakmehrotra.com/paper_trader/account/delete/portfolioList?id=${accountID}&portfolioID=${portfolioToDelete.portfolioID}`, {
          method: "DELETE"
        });
      } else {
