...
export default function PortfolioDetails() {
  ...
  const fetchAllPortfolios = () => {
    fetch(`https://parcel-sides-effort-italiano.trycloudflare.compaper_trader/account/get/portfolioList?id=${accountID}`)
      .then(response => response.json())
      .then(data => {
        if (Array.isArray(data)) {
          const portfolioPromises = data.map(id =>
            fetch(`https://parcel-sides-effort-italiano.trycloudflare.compaper_trader/portfolio/get/name?id=${id}`)
              .then(response => {
                if (response.ok) {
  ...
  const fetchPortfolioDetails = (id) => {
    fetch(`https://parcel-sides-effort-italiano.trycloudflare.compaper_trader/portfolio/get?id=${id}`)
      .then(response => response.json())
      .then(data => {
        setPortfolio(data);
        if (data.assets) {
  ...
  const fetchAssetPrices = (assets, cashAmount) => {
    const assetPromises = Object.keys(assets).map(asset => {
      if (asset === "Cash") {
        ...
        return fetch(`https://parcel-sides-effort-italiano.trycloudflare.compaper_trader/polygon/price?ticker=${asset}`)
          .then(response => {
            if (!response.ok) {
  ...
  const fetchTransactions = (transactionIDs) => {
    const transactionPromises = transactionIDs.map(id =>
      fetch(`https://parcel-sides-effort-italiano.trycloudflare.compaper_trader/transaction/get?id=${id}`)
        .then(response => response.json())
    );
  ...
  const handleConfirmDelete = () => {
    fetch(`https://parcel-sides-effort-italiano.trycloudflare.compaper_trader/portfolio/remove?id=${portfolioToDelete.portfolioID}`, {
      method: "DELETE"
    })
    .then(response => {
      if (response.ok) {
        return fetch(`https://parcel-sides-effort-italiano.trycloudflare.compaper_trader/account/delete/portfolioList?id=${accountID}&portfolioID=${portfolioToDelete.portfolioID}`, {
          method: "DELETE"
        });
      } else {
