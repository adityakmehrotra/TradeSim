...
export const api = {
  signup: async (userData, username, password) => {
    try {
      const response = await fetch(
        `${API_BASE_URL}/paper_trader/account/add?username=${encodeURIComponent(username)}&password=${encodeURIComponent(password)}`,
        {
          method: 'POST',
          headers: {
  ...
  login: async (username, password) => {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/paper_trader/account/login?${params.toString()}`,
        {
          method: 'POST',
          headers: {
